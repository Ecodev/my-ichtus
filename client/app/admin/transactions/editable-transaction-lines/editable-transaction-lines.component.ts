...
> {
    ...
    /**
     * Whether the field belongs to the accounting closing that generated the line and is locked
     */
    protected isFieldLocked(line: AbstractControl, field: string): boolean {
        return (line.get('lockedFields')?.value as string[] | undefined)?.includes(field) ?? false;
    }

    /**
     * A line generated by an accounting closing cannot be deleted, the server refusing it
     */
    protected canRemove(line: AbstractControl): boolean {
        return !(line.get('lockedFields')?.value as string[] | undefined)?.length;
    }
